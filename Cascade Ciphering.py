...
def OTP_Encryption(m,k):
    ...
    for index, m_index in enumerate(m): #Assigns two variables and scans the characters of the plaintext.
        ...
        cipher_index = (msg_index + key_index) % 26 #OTP's Mathematical equation for Encryption.
        print("OTP Encryption Equation Result:", cipher_index, "=", chr(cipher_index + 97))
        Ciphertxt = Ciphertxt + alphabit[cipher_index] #Appends the encrypted characters into a string.
        print("OTP Cipher Text:", Ciphertxt)
        print("- - - -")
    return (Ciphertxt)

def OTP_Decryption(c, k):
    global alphabit
    ...
    for index, c_index in enumerate(c):
        ...
        print("Key index:", key_index)
        cipher_index = (ciphermsg_index - key_index) % 26 #OTP's Mathematical equation for Decryption.
        print("OTP Decryption Equation Result:", cipher_index, "=", chr(cipher_index + 97))
        Decryptiontxt = Decryptiontxt + alphabit[cipher_index] #Appends the decrypted characters into a string.
        print("OTP Decrypted Text:", Decryptiontxt)
        print("- - - -")
...
def calculate():
    plain_text = plain_text_entry.get() #I love UOT IRAQ
    key = key_entry.get() # malikabdulahslow
    padded_msg = Buffer(plain_text)
    ECP = OTP_Encryption(padded_msg, key) 
    ...
    state = [int(x, 16) for x in output_hex]
    ...
    otp_decdec=OTP_Decryption(inv_ascii, key)
    otp_decdec = otp_decdec.replace('z', '')
    Hash_phrase = "Milky Way"
    MD5_Hash = md5(Hash_phrase)

    padding.config(text=f"Padded Message: {padded_msg}")
    otp_encryption.config(text=f"OTP Encryption: {ECP}")
    aes_encryption.config(text=f"AES Encryption: {Enc_msg_ASCII}")
    md5_output.config(text=f"MD5 Hash: {MD5_Hash}")
    plusmd5_output.config(text=f"Enc msg + MD5 Hash:{Enc_msg_ASCII}{MD5_Hash}")
    aes_decryption.config(text=f"AES Decryption: {inv_ascii}")
    otp_decryption.config(text=f"Original Message (OTP Decryption): {otp_decdec}")
    
    
    
root = tk.Tk()
root.title("Cascade Ciphering")

root.minsize(600, 400)

plain_text_label = tk.Label(root, text="Plain Text:")
plain_text_label.grid(row=0, column=0, padx=(20, 5), pady=(20, 5), sticky=tk.W)
plain_text_entry = tk.Entry(root)
plain_text_entry.grid(row=0, column=1, padx=(5, 20), pady=(20, 5), sticky=tk.EW)

key_label = tk.Label(root, text="OTP Key:")
key_label.grid(row=1, column=0, padx=(20, 5), pady=(5, 5), sticky=tk.W)
key_entry = tk.Entry(root)
key_entry.grid(row=1, column=1, padx=(5, 20), pady=(5, 5), sticky=tk.EW)

calculate_button = tk.Button(root, text="Calculate", command=calculate)
calculate_button.grid(row=2, column=0, columnspan=2, padx=20, pady=(5, 20), sticky=tk.EW)

padding = tk.Label(root, text="Padded Message: ", wraplength=400)
padding.grid(row=3, column=0, columnspan=2, padx=20, pady=5, sticky=tk.W)

otp_encryption = tk.Label(root, text="OTP Encryption: ", wraplength=400)
otp_encryption.grid(row=4, column=0, columnspan=2, padx=20, pady=5, sticky=tk.W)

aes_encryption = tk.Label(root, text="AES Encryption: ", wraplength=400)
aes_encryption.grid(row=5, column=0, columnspan=2, padx=20, pady=5, sticky=tk.W)

md5_output = tk.Label(root, text="MD5 Hash: ", wraplength=400)
md5_output.grid(row=6, column=0, columnspan=2, padx=20, pady=5, sticky=tk.W)

plusmd5_output = tk.Label(root, text="Enc msg + MD5 Hash: ", wraplength=400)
plusmd5_output.grid(row=7, column=0, columnspan=2, padx=20, pady=5, sticky=tk.W)

aes_decryption = tk.Label(root, text="AES Decryption: ", wraplength=400)
aes_decryption.grid(row=8, column=0, columnspan=2, padx=20, pady=5, sticky=tk.W)

otp_decryption = tk.Label(root, text="Original Message (OTP Decryption): ", wraplength=400)
otp_decryption.grid(row=9, column=0, columnspan=2, padx=20, pady=5, sticky=tk.W)
